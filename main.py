    ...
    def pre_stock(self):
        ...
        for i, symbol in enumerate(self.data):
            ...
            if change == []:
                ...
                price = float('{:.4f}'.format(
                    round(price, 4)).rstrip('0').rstrip('.') or '0')
                stock_dict['Change'] = float('{:.4f}'.format(
                    round(change, 4)).rstrip('0').rstrip('.') or '0')
            count_pass += 1
            ...
            if self.index == "NASDAQ":
                if self.filters != []:
                    try:
                        ...
                        continue
                    key_dict = {'Report EPS': 0, 'Total Asset': 1, 'Liabilities': 2,
                                'Gross Profit': 3, 'Total Revenue': 4, 'Net Income': 5}
                ...
                if self.filters != []:
                    try:
                        ...
                        continue
                    key_dict = {'Total Asset': 0, 'Liabilities': 1, 'Equity': 2, 'Paid Up Capital': 3, 'Revenue': 4, 'Net Profit': 5, 'EPS': 6,
                                'ROA': 7, 'ROE': 8, 'Net Profit Margin': 9, 'Market Capitalization': 10, 'P/E': 11, 'P/BE': 12, 'Dividend Yield': 13}
                    key_list = self.filters
                    for j in key_list:
                        stock_dict[j[0]] = financial[key_dict[j[0]]+3]
...
class MyThread_crypto(QThread):
    ...
    def search_crypto(self):
        ...
        for sym in all_symbol_crypto:
            ...
            if change == []:
                continue
            change = float('{:.5f}'.format(round(change, 5)
                                           ).rstrip('0').rstrip('.') or '0')
            if price == [] or change == []:
                continue
            else:
                filtered_data.append(
                    dict(symbol=sym, Price=price, Change=change, name=""))
...
class MyThread_treemap(QThread):
    ...
    @QtCore.pyqtSlot()
    def tree(self):
        global symbol, sector, industry, market_cap, name
        symbol = []
        name = []
        sector = []
        industry = []
        ...
        if self.index == "CRYPTO":
            ...
            fig = px.treemap(
                df, path=['Coin'], values='Market Capitalization', custom_data=[df['Name']])
    ...
    def process_symbol(self, i, index):
        ...
        sector = obj.sector()
        industry = obj.industry()
        name = obj.get_stock_name()
        return (i, name, sector, industry, temp_market_cap)
    ...
    def process_symbols_batch(self, start_index, end_index, index):
        for i in range(start_index, end_index):
            ...
            if result is not None:
                ...
                sector.append(result[2])
                industry.append(result[3])
                market_cap.append(result[4])
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ui.fig2.setZoomFactor(0.6)
        ...
        self.ui.symbol_comboBox.completer().setCompletionMode(
        ...
        # change completion mode of the default completer from InlineCompletion to PopupCompletion
        self.ui.symbol_comboBox_2.completer().setCompletionMode(
            QtWidgets.QCompleter.PopupCompletion)
        ...
        self.ui.symbol_comboBox.lineEdit().setPlaceholderText("Enter Stock Symbol")
        self.ui.symbol_comboBox_2.lineEdit().setPlaceholderText("Enter Stock Symbol")
        self.ui.symbol_comboBox.lineEdit().textChanged.connect(self.on_text_changed_combo_1)
        self.ui.symbol_comboBox_2.lineEdit().textChanged.connect(
            self.on_text_changed_combo_2)
        ...
        self.ui.symbol_comboBox.currentIndexChanged.connect(
            self.change_stock_graph)
        ...
        self.ui.freq_comboBox_2.currentIndexChanged.connect(
            self.change_interval)

        self.ui.statements_comboBox.setCurrentIndex(-1)
        self.ui.statements_comboBox.currentIndexChanged.connect(
            self.change_finance)

        self.ui.statements_comboBox_2.setCurrentIndex(-1)
        self.ui.statements_comboBox_2.currentIndexChanged.connect(
            self.change_finance)
        ...
        self.ui.button_overall_page.clicked.connect(
            lambda: self.click_overall())
        self.ui.button_stock_page.clicked.connect(lambda: self.click_stock())
        self.ui.button_financial_page.clicked.connect(
            lambda: self.click_financial())
        self.ui.button_spatial_page.clicked.connect(
            lambda: self.click_spatial())
        ...
        self.ui.button_update_all_page.clicked.connect(
            lambda: self.click_update_all())
        self.ui.pushButton.clicked.connect(lambda: self.click_add_page())

        self.ui.crypto_tableWidget.setColumnCount(3)
        self.ui.crypto_tableWidget.setHorizontalHeaderLabels(
            ['Symbol', 'Price', 'Change'])
        self.ui.crypto_tableWidget.horizontalHeader(
        ).setSectionResizeMode(QHeaderView.Stretch)
        self.ui.crypto_tableWidget.setSizeAdjustPolicy(
            QAbstractScrollArea.AdjustToContents)
        self.ui.crypto_tableWidget.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Expanding)

        # #-------------watch search crypto------------------------------
        self.ui.search_button_crypto.clicked.connect(self.search_crypto_buff)
        self.ui.sort_comboBox_crypto2.currentIndexChanged.connect(
            lambda: self.show_crypto(self.last_crypto_data))

        # #-------------watch search set------------------------------
        self.ui.search_button_set.clicked.connect(self.search_stock_buff_set)
        self.ui.sort_comboBox_set1.currentIndexChanged.connect(
            lambda: self.show_stock_set(self.last_stock_filtered_data_set))
        self.ui.sort_comboBox_set2.currentIndexChanged.connect(
            lambda: self.show_stock_set(self.last_stock_filtered_data_set))

        # ------------------------treemap---------------------------
        self.ui.search_button_set_2.clicked.connect(self.plot_treemap_set)
        self.ui.search_button_nasdaq_2.clicked.connect(
            self.plot_treemap_nasdaq)
        self.ui.search_button_crypto_2.clicked.connect(
            self.plot_treemap_crypto)

        # #-------------watch search nasdaq------------------------------
        self.ui.search_button_nasdaq.clicked.connect(
            self.search_stock_buff_nasdaq)
        self.ui.sort_comboBox_nasdaq1.currentIndexChanged.connect(
            lambda: self.show_stock_nasdaq(self.last_stock_filtered_data_nasdaq))
        self.ui.sort_comboBox_nasdaq2.currentIndexChanged.connect(
            lambda: self.show_stock_nasdaq(self.last_stock_filtered_data_nasdaq))
        ...
        # # filter set
        self.ui.industry_comboBox_set.currentIndexChanged.connect(
            lambda: self.choose_sec_indus_only_one_set("indus"))
        self.ui.sector_comboBox_set.currentIndexChanged.connect(
            lambda: self.choose_sec_indus_only_one_set("sec"))
        self.ui.industry_comboBox_set_2.currentIndexChanged.connect(
            lambda: self.choose_sec_indus_only_one_set_2("indus"))
        self.ui.sector_comboBox_set_2.currentIndexChanged.connect(
            lambda: self.choose_sec_indus_only_one_set_2("sec"))
        self.ui.filter_tableWidget_set.setColumnCount(4)
        self.ui.filter_tableWidget_set.setRowCount(0)
        self.ui.filter_tableWidget_set.setHorizontalHeaderLabels(
            ["Filter", "Minimum value", "Maximum value", "Remove"])
        self.ui.add_filter_button_set.clicked.connect(self.add_filter_set)
        self.ui.filter_tableWidget_set.horizontalHeader(
        ).setSectionResizeMode(QHeaderView.Stretch)
        self.ui.filter_tableWidget_set.setSizeAdjustPolicy(
            QAbstractScrollArea.AdjustToContents)
        self.ui.filter_tableWidget_set.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Expanding)

        # # filter nasdaq
        self.ui.industry_comboBox_nasdaq.currentIndexChanged.connect(
            lambda: self.choose_sec_indus_only_one_nasdaq("indus"))
        self.ui.sector_comboBox_nasdaq.currentIndexChanged.connect(
            lambda: self.choose_sec_indus_only_one_nasdaq("sec"))
        self.ui.industry_comboBox_nasdaq_2.currentIndexChanged.connect(
            lambda: self.choose_sec_indus_only_one_nasdaq_2("indus"))
        self.ui.sector_comboBox_nasdaq_2.currentIndexChanged.connect(
            lambda: self.choose_sec_indus_only_one_nasdaq_2("sec"))
        self.ui.filter_tableWidget_nasdaq.setColumnCount(4)
        self.ui.filter_tableWidget_nasdaq.setRowCount(0)
        self.ui.filter_tableWidget_nasdaq.setHorizontalHeaderLabels(
            ["Filter", "Minimum value", "Maximum value", "Remove"])
        self.ui.add_filter_button_nasdaq.clicked.connect(
            self.add_filter_nasdaq)
        self.ui.filter_tableWidget_nasdaq.horizontalHeader(
        ).setSectionResizeMode(QHeaderView.Stretch)
        self.ui.filter_tableWidget_nasdaq.setSizeAdjustPolicy(
            QAbstractScrollArea.AdjustToContents)
        self.ui.filter_tableWidget_nasdaq.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Expanding)

        # #stock set
        self.ui.symbol_tableWidget_set.horizontalHeader(
        ).setSectionResizeMode(QHeaderView.Stretch)
        self.ui.symbol_tableWidget_set.setSizeAdjustPolicy(
            QAbstractScrollArea.AdjustToContents)
        self.ui.symbol_tableWidget_set.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Expanding)

        # # #stock nasdaq
        self.ui.symbol_tableWidget_nasdaq.horizontalHeader(
        ).setSectionResizeMode(QHeaderView.Stretch)
        self.ui.symbol_tableWidget_nasdaq.setSizeAdjustPolicy(
            QAbstractScrollArea.AdjustToContents)
        self.ui.symbol_tableWidget_nasdaq.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        self.ui.tableWidget.setSizeAdjustPolicy(
            QAbstractScrollArea.AdjustToContents)
        self.ui.tableWidget.setSizePolicy(
            QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.ui.start_download_button.clicked.connect(self.update_all)

        # check
        self.ui.check_button.clicked.connect(self.insert_new_symbol)
        self.ui.symbol_textLine.returnPressed.connect(self.insert_new_symbol)
        self.ui.symbol_textLine.textChanged.connect(self.on_text_changed)
...
# -----------------------------upper--------------------------------------------------------
    def on_text_changed_combo_1(self, text):
        self.ui.symbol_comboBox.lineEdit().blockSignals(True)
        upper_text = text.upper()
        if text != upper_text:
            self.ui.symbol_comboBox.lineEdit().setText(upper_text)
            self.ui.symbol_comboBox.lineEdit().setCursorPosition(len(upper_text))
        self.ui.symbol_comboBox.lineEdit().blockSignals(False)

    def on_text_changed_combo_2(self, text):
        self.ui.symbol_comboBox_2.lineEdit().blockSignals(True)
        upper_text = text.upper()
        if text != upper_text:
            self.ui.symbol_comboBox_2.lineEdit().setText(upper_text)
            self.ui.symbol_comboBox_2.lineEdit().setCursorPosition(len(upper_text))
        self.ui.symbol_comboBox_2.lineEdit().blockSignals(False)
# -----------------------------Add new symbol-----------------------------------------------------------------------------------

    def insert_new_symbol(self):
        index = self.ui.Index_comboBox.currentText()
        symbol = self.ui.symbol_textLine.text()
        if symbol == '':
            return
        self.ui.frame.setVisible(True)
        state = Categories(index).insert_new_stock(symbol)
        if state == 'fail':
            self.ui.label.setText(symbol+" doesn't exist in "+index+" index")
        elif state == 'have':
            self.ui.label.setText(symbol+" is already in database")
        else:
            self.ui.label.setText(symbol+" added!")
            Set = Categories('SET')
            Nasdaq = Categories('NASDAQ')
            Crypto = Categories('CRYPTO')
            all_symbol = []
            ...
            for symbol in Crypto.get_all_stock():
                if len(symbol) > 4:
                    ...
                    all_symbol.append(symbol+'-USD'+'\t\t- CRYPTO')
            self.ui.symbol_comboBox.blockSignals(True)
            self.ui.symbol_comboBox_2.blockSignals(True)
            self.ui.symbol_comboBox.clear()
            self.ui.symbol_comboBox_2.clear()
            self.ui.symbol_comboBox.addItems(all_symbol)
            self.ui.symbol_comboBox_2.addItems(all_symbol)
            self.ui.symbol_comboBox.blockSignals(False)
            self.ui.symbol_comboBox_2.blockSignals(False)

    def on_text_changed(self, text):
        self.ui.symbol_textLine.blockSignals(True)
        upper_text = text.upper()
        if text != upper_text:
            self.ui.symbol_textLine.setText(upper_text)
            self.ui.symbol_textLine.setCursorPosition(len(upper_text))
        self.ui.symbol_textLine.blockSignals(False)
...
# #------------------------------------Watch Filter-------------------------------------------------------------
    def add_filter_set(self):
        ...
        self.ui.filter_tableWidget_set.setItem(
            row, 0, QTableWidgetItem(filter_name))
        self.ui.filter_tableWidget_set.setItem(
            row, 1, QTableWidgetItem(str(lower_bound)))
        self.ui.filter_tableWidget_set.setItem(
            row, 2, QTableWidgetItem(str(upper_bound)))

        # Create remove button for row
        button_remove = QPushButton("Remove")
        button_remove.clicked.connect(
            lambda checked, row=row: self.delete_row_set(row))
    ...
    def add_filter_nasdaq(self):
        ...
        self.ui.filter_tableWidget_nasdaq.setItem(
            row, 0, QTableWidgetItem(filter_name))
        self.ui.filter_tableWidget_nasdaq.setItem(
            row, 1, QTableWidgetItem(str(lower_bound)))
        self.ui.filter_tableWidget_nasdaq.setItem(
        ...
        button_remove = QPushButton("Remove")
        button_remove.clicked.connect(
            lambda checked, row=row: self.delete_row_nasdaq(row))
    ...
    def delete_row_nasdaq(self, row):
        ...
        for row in range(self.ui.filter_tableWidget_nasdaq.rowCount()):
            ...
            button_remove.clicked.connect(
                lambda checked, row=row: self.delete_row_nasdaq(row))
            self.ui.filter_tableWidget_nasdaq.setCellWidget(
                row, 3, button_remove)

    def load_all_sector_indus(self, index):  # load to dropdown
        if index == "SET":
            self.ui.sector_comboBox_set.addItems(sorted(get_all_sector(index)))
            self.ui.industry_comboBox_set.addItems(
                sorted(get_all_industry(index)))
            self.ui.sector_comboBox_set_2.addItems(
                sorted(get_all_sector(index)))
            self.ui.industry_comboBox_set_2.addItems(
                sorted(get_all_industry(index)))
        else:
            self.ui.sector_comboBox_nasdaq.addItems(
                sorted(get_all_sector(index)))
            self.ui.industry_comboBox_nasdaq.addItems(
                sorted(get_all_industry(index)))
            self.ui.sector_comboBox_nasdaq_2.addItems(
                sorted(get_all_sector(index)))
            self.ui.industry_comboBox_nasdaq_2.addItems(
                sorted(get_all_industry(index)))
...
# ------------------------------------------Tree Map-----------------------------------------------------------------
    ...
    def plot_treemap_nasdaq(self):
        ...
        self.thread_nasdaq.finished.connect(
            self.thread_nasdaq_thread.quit)  # added
        self.thread_nasdaq.finished.connect(
            self.thread_nasdaq.deleteLater)  # added
        self.thread_nasdaq.treemap_ready.connect(self.show_treemap_nasdaq)
        self.thread_nasdaq_thread.started.connect(
            self.start_worker_treemap_nasdaq)  # moved
    ...
    def start_worker_treemap_nasdaq(self):
        ...
        QtCore.QMetaObject.invokeMethod(
            self.thread_nasdaq, 'tree', QtCore.Qt.QueuedConnection)
    ...
    def plot_treemap_crypto(self):
        ...
        self.thread_crypto.finished.connect(
            self.thread_crypto_thread.quit)  # added
        self.thread_crypto.finished.connect(
            self.thread_crypto.deleteLater)  # added
        self.thread_crypto.treemap_ready.connect(self.show_treemap_crypto)
        self.thread_crypto_thread.started.connect(
            self.start_worker_treemap_crypto)  # moved
    ...
    def start_worker_treemap_crypto(self):
        ...
        self.ui.search_button_crypto_2.setEnabled(False)
        QtCore.QMetaObject.invokeMethod(
            self.thread_crypto, 'tree', QtCore.Qt.QueuedConnection)
...
#     #-----------set--------------
    ...
    def start_worker_set(self):
        self.ui.search_button_set.setEnabled(False)
        QtCore.QMetaObject.invokeMethod(
            self.worker_set, 'pre_stock', QtCore.Qt.QueuedConnection)
    ...
    def show_stock_set(self, filtered_data):
        ...
        if sort_order == 0:
            ...
            sorted_data = sorted(
                filtered_data, key=lambda x: x[sort_topic], reverse=True)
        ...
        remaining_keys = sorted_data[0].keys(
        ) - first_keys - exclude_keys - last_keys

        # Convert the unique_keys set into a list in the desired order
        column_order = ['Symbol', 'Price'] + \
            list(remaining_keys) + list(last_keys)
    ...
    def search_stock_buff_nasdaq(self):
        ...
        self.worker_nasdaq.finished.connect(
            self.worker_thread_nasdaq.quit)  # added
        self.worker_nasdaq.finished.connect(
            self.worker_nasdaq.deleteLater)  # added
        self.worker_nasdaq.result_ready.connect(
            self.get_last_stock_price_nasdaq)
    ...
    def start_worker_nasdaq(self):
        self.ui.search_button_nasdaq.setEnabled(False)
        QtCore.QMetaObject.invokeMethod(
            self.worker_nasdaq, 'pre_stock', QtCore.Qt.QueuedConnection)
    ...
    def show_stock_nasdaq(self, filtered_data):
        ...
        # Convert the unique_keys set into a list in the desired order
        column_order = ['Symbol', 'Price'] + \
            sorted(list(remaining_keys)) + list(last_keys)
        ...
        self.ui.symbol_tableWidget_nasdaq.setHorizontalHeaderLabels(
            column_order)
...
# # -----------------------------------Crypto Watch---------------------------------------------------------------
    def search_crypto_buff(self):
        ...
        self.worker_cryp.finished.connect(
            self.worker_thread_cryp.quit)  # added
        self.worker_cryp.finished.connect(
            self.worker_cryp.deleteLater)  # added
    ...
    def start_worker_crypto(self):
        ...
        QtCore.QMetaObject.invokeMethod(
            self.worker_cryp, 'search_crypto', QtCore.Qt.QueuedConnection)
    ...
    def show_crypto(self, data):
        ...
        if sort_order == 0:
            ...
            sorted_data = sorted(
                data, key=lambda x: x[sort_topic], reverse=True)
        ...
        if self.current_data != None:
            for i in range(round):
                if i == 0 and price_state:
                    ...
                    self.worker = MyThread_download(
                        [self.current_data[0]], [self.current_data[1]])
                    self.worker.moveToThread(self.worker_thread)
                    self.worker.finished.connect(
                        self.worker_thread.quit)  # added
                    self.worker.finished.connect(
                        self.worker.deleteLater)  # added
                    self.worker.finished.connect(
                        lambda: self.update_data("price", False))
                    self.worker_thread.started.connect(
                        self.start_worker_download_price)  # moved
                    ...
                    self.worker_statement = MyThread_download(
                        [self.current_data[0]], [self.current_data[1]])
                    self.worker_statement.moveToThread(
                        self.worker_thread_statement)
                    self.worker_statement.finished.connect(
                        self.worker_thread_statement.quit)  # added
                    self.worker_statement.finished.connect(
                        self.worker_statement.deleteLater)  # added
                    self.worker_statement.finished.connect(
                        lambda: self.update_data("statement", False))
                    self.worker_thread_statement.started.connect(
                        self.start_worker_download_statement)  # moved
                    self.worker_thread_statement.start()
                    self.worker_statement.finished.connect(
                        self.worker_thread_statement.quit)
                    self.worker_statement.finished.connect(
                        self.worker_thread_statement.wait)
                elif i == 2 and news_state:
                    self.load_number += 1
                    self.worker_thread_news = QThread()
                    self.worker_news = MyThread_download(
                        [self.current_data[0]], [self.current_data[1]])
                    self.worker_news.moveToThread(self.worker_thread_news)
                    self.worker_thread_news.finished.connect(
                        self.worker_thread_news.quit)  # added
                    self.worker_thread_news.finished.connect(
                        self.worker_news.deleteLater)  # added
                    self.worker_news.finished.connect(
                        lambda: self.update_data("news", location_state))
                    self.worker_thread_news.started.connect(
                        self.start_worker_download_news)  # moved
                    self.worker_thread_news.start()
                    self.worker_news.finished.connect(
                        self.worker_thread_news.quit)
                    self.worker_news.finished.connect(
                        self.worker_thread_news.wait)
                elif i == 3 and location_state:
                    self.load_number += 1
                    self.worker_thread_location = QThread()
                    self.worker_location = MyThread_download(
                        [self.current_data[0]], [self.current_data[1]])
                    self.worker_location.moveToThread(
                        self.worker_thread_location)
                    self.worker_location.finished.connect(
                        self.worker_thread_location.quit)  # added
                    self.worker_location.finished.connect(
                        self.worker_location.deleteLater)  # added
                    self.worker_location.finished.connect(
                        lambda: self.update_data("location", False))
                    self.worker_thread_location.started.connect(
                        self.start_worker_download_location)  # moved
                    self.worker_thread_location.start()
                    self.worker_location.finished.connect(
                        self.worker_thread_location.quit)
                    self.worker_location.finished.connect(
                        self.worker_thread_location.wait)

    def start_worker_download_price(self):
        self.ui.update_button.setEnabled(False)
        QtCore.QMetaObject.invokeMethod(
            self.worker, 'update_data_price', QtCore.Qt.QueuedConnection)

    def start_worker_download_statement(self):
        self.ui.update_button.setEnabled(False)
        QtCore.QMetaObject.invokeMethod(
            self.worker_statement, 'update_data_statement', QtCore.Qt.QueuedConnection)

    def start_worker_download_news(self):
        self.ui.update_button.setEnabled(False)
        QtCore.QMetaObject.invokeMethod(
            self.worker_news, 'update_data_news', QtCore.Qt.QueuedConnection)

    def start_worker_download_location(self):
        self.ui.update_button.setEnabled(False)
        QtCore.QMetaObject.invokeMethod(
            self.worker_location, 'update_data_location', QtCore.Qt.QueuedConnection)

    def start_worker_download_location2(self):
        self.ui.update_button.setEnabled(False)
        QtCore.QMetaObject.invokeMethod(
            self.worker_location2, 'update_data_location', QtCore.Qt.QueuedConnection)

    def update_data(self, who, state):
        part = 100/self.load_number
        if state:
            self.worker_thread_location2 = QThread()
            self.worker_location2 = MyThread_download(
                [self.current_data[0]], [self.current_data[1]])
            self.worker_location2.moveToThread(self.worker_thread_location2)
            self.worker_location2.finished.connect(
                self.worker_thread_location2.quit)  # added
            self.worker_location2.finished.connect(
                self.worker_location2.deleteLater)  # added
            self.worker_location2.finished.connect(
                lambda: self.update_data("location", False))
            self.worker_thread_location2.started.connect(
                self.start_worker_download_location2)  # moved
            self.worker_thread_location2.start()
            self.worker_location2.finished.connect(
                self.worker_thread_location2.quit)
            self.worker_location2.finished.connect(
                self.worker_thread_location2.wait)

        if who == "price":
            self.progress += part
            self.ui.progressBar.setValue(int(self.progress))
            self.finish += 1
            self.pic_candle = plot_candle(
                self.current_data[0], self.current_data[1])
            ...
            if self.last_candle_key == "No data" and len(interval_list) > 1:
                ...
                self.ui.fig.setHtml(
                    self.pic_candle[self.last_candle_key].to_html(include_plotlyjs='cdn'))
                self.ui.fig4.setHtml(
                    self.pic_candle[self.last_candle_key].to_html(include_plotlyjs='cdn'))
            else:
                self.ui.fig.setHtml(
                    self.pic_candle[self.last_candle_key].to_html(include_plotlyjs='cdn'))
            ...
            all_news = get_all_news(
                self.current_data[0], self.current_data[1], "All")
            ...
            self.ui.progressBar.setValue(int(self.progress))
            self.finish += 1
            self.pic_spatial = plot_spatial(
                self.current_data[0], self.current_data[1])
            self.ui.fig3.setHtml(
                self.pic_spatial["All"].to_html(include_plotlyjs='cdn'))
            self.ui.fig6.setHtml(
                self.pic_spatial["All"].to_html(include_plotlyjs='cdn'))
...
# #-----------------------------------------update all------------------------------------------------------------
    ...
    def add_symbol(self):
        ...
        self.ui.tableWidget.setItem(
            row_count, 0, QtWidgets.QTableWidgetItem(symbol))
        self.ui.tableWidget.setItem(
            row_count, 1, QtWidgets.QTableWidgetItem(category))

        button_remove = QPushButton("Remove")
        button_remove.clicked.connect(
            lambda checked, row=row_count: self.delete_symbol(row_count))
    ...
    def delete_symbol(self, row):
        ...
        for row in range(self.ui.tableWidget.rowCount()):
            button_remove = QPushButton("Remove")
            button_remove.clicked.connect(
                lambda checked, row=row: self.delete_symbol(row))
    ...
    def update_all(self):
        ...
        self.worker_update_all.finished.connect(
            self.worker_update_all_thread.quit)
        self.worker_update_all.finished.connect(
            self.worker_update_all.deleteLater)
        self.worker_update_all.ready.connect(self.update_all_finished)
        self.worker_update_all.error.connect(self.error)
        self.worker_update_all_thread.started.connect(
            self.start_worker_download_all)
        self.worker_update_all_thread.start()
        self.worker_update_all.finished.connect(
            self.worker_update_all_thread.quit)
        self.worker_update_all.finished.connect(
            self.worker_update_all_thread.wait)

    def start_worker_download_all(self):
        self.ui.start_download_button.setEnabled(False)
        QtCore.QMetaObject.invokeMethod(
            self.worker_update_all, 'update_data_price', QtCore.Qt.QueuedConnection)
    ...
    def update_all_finished(self):
        ...
        if self.finish2 == self.number_symbol_download:
            ...
            self.ui.tableWidget.setHorizontalHeaderLabels(
                ["Symbol(Undownloadable "+str(len(self.uncomplete))+" symbols)", "Index (Undownloadable)"])
...
# #-----------------------------------------change graph---------------------------------------------------------
    ...
    def change_finance(self, index):
        ...
        if len(list(self.pic_finance.keys())) > 1:
            self.ui.fig2.setHtml(
                self.pic_finance[key][0].to_html(include_plotlyjs='cdn'))
            self.ui.fig5.setHtml(
                self.pic_finance[key][0].to_html(include_plotlyjs='cdn'))
    ...
    def change_period(self, index):
        ...
        self.ui.fig3.setHtml(
            self.pic_spatial[key].to_html(include_plotlyjs='cdn'))
        self.ui.fig6.setHtml(
            self.pic_spatial[key].to_html(include_plotlyjs='cdn'))
        ...
        all_news = get_all_news(
            self.current_data[0], self.current_data[1], key)
        for each_news in all_news:
            self.add_news_item(each_news[1], each_news[2], each_news[4])
